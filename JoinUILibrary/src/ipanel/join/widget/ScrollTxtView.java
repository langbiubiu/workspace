...
public class ScrollTxtView extends ScrollTextView implements IConfigView, IConfigRebind{
	...
	public ScrollTxtView(Context context, View data) {
		super(context);
		this.mData = data;
		bindProperty(data);
	}

	public ScrollTxtView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
	}
	public ScrollTxtView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	private void bindProperty(View data) {
		...
		bind = data.getBindByName(PROP_SCROLL_DIRECTION);
		if(bind != null){
			setScrollDirection(ScrollDirection.valueOf(bind.getValue().getvalue()));
		}
		bind = data.getBindByName(PROP_SCROLL_SPEED);
		if (bind != null) {
			setSpeed(Float.parseFloat(bind.getValue().getvalue())
