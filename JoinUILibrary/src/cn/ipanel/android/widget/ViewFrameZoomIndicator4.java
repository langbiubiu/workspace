...
public class ViewFrameZoomIndicator4 implements IFrameIndicator {
	private ImageView mImg;
	private FrameLayout.LayoutParams lastEnd;
	...
	public ViewFrameZoomIndicator4(Activity context) {
	...
	public ViewFrameZoomIndicator4(FrameLayout frameLayout) {
	...
	public void setPadding(int left, int top, int right, int bottom) {
		...
		Logger.d("left:" + mImg.getPaddingLeft() + "," + mImg.getPaddingRight()
				+ "," + mImg.getPaddingTop() + "," + mImg.getPaddingBottom());
	}
	...
	private int mAnimationTime = 92;
	private int mAnimationTime2 = 0;
	private int mAnimationTime1 = 90;
	private int mAnimationTime11 = 1;
	...
	public void moveFrameTo(final View ve, boolean animated, boolean hideFrame) {
		...
		if (ve != null) {
			...
			Logger.d("left:" + follower.getPaddingLeft() + ","
					+ follower.getPaddingRight() + ","
					+ follower.getPaddingTop() + ","
					+ follower.getPaddingBottom());
			Logger.d("left:" + mImg.getPaddingLeft() + ","
					+ mImg.getPaddingRight() + "," + mImg.getPaddingTop() + ","
					+ mImg.getPaddingBottom());
			...
			if (animated && !hideFrame && wasVisible) {
				...
				v.postDelayed(mScaleRunable, 65);
				// startScaleAnimation(follower,v,w,h);
			} else if (animated && !hideFrame) {
				follower.setLayoutParams(end);
				if (!hideFrame)
					follower.setVisibility(View.VISIBLE);
				follower.setVisibility(View.VISIBLE);
				follower.setAlpha(0f);
				follower.animate().alpha(1f).setDuration(mAnimationTime2)
						.start();
				// startScaleAnimation(follower,v,w,h);
				v.removeCallbacks(mScaleRunable);
				v.postDelayed(mScaleRunable, 65);
	...
	private void startScaleAnimation(View follower, View v, int w, int h) {
		ScaleAnimation animation = new ScaleAnimation(1f, mTargetScale, 1f,
				mTargetScale, Animation.RELATIVE_TO_SELF, 0.5f,
				Animation.RELATIVE_TO_SELF, 0.5f);

		ScaleAnimation animation2 = new ScaleAnimation(1f, (mTargetScale * w
				+ v.getPaddingLeft() + v.getPaddingRight())
				/ (w + v.getPaddingLeft() + v.getPaddingRight()), 1f,
				(mTargetScale * h + v.getPaddingTop() + v.getPaddingBottom())
		...
		animation.setDuration(mAnimationTime1);
		animation.setFillAfter(true);
		animation.setInterpolator(new AnticipateInterpolator());
		animation2.setDuration(mAnimationTime11);
