...
public class HomedProgramPlayer extends TeeveePlayerBase implements HomedProgramPlayerInterface,
		HomedProgramPlayerInterface.Callback {
	final static String TAG = HomedProgramPlayer.class.getSimpleName();
	public static final int WIDGET_FALG_TIPS = 0x01;
	static final int GROUP = 0, CHANNEL = 1, PRESENT = 2, FOLLOW = 3;

	public HomedProgramPlayer(Context context, String serviceName) {
	...
	public final void select(long freq, int fflags, int program, int pflags) {
		try {
			...
			str.endObject();
			channel.transmit(__ID_homed_select, str.toString(), null);/*-update version;*/
	...
	public void startFd(long vfreq, ParcelFileDescriptor pfd, int fflags) {
		try {
			...
			channel.transmit(__ID_homed_pfd_start, str.toString(), p);/*-update version;*/
		} catch (JSONException e) {
			throw new RuntimeException(e.getMessage());
		}
	}
	...
	public final void select(String furi, int fflags, String puri, int pflags) {
		try {
			...
			str.key("pflags").value(pflags);
			str.endObject();
			channel.transmit(__ID_homed_select_2, str.toString(), null);/*-update version;*/
		} catch (JSONException e) {
			throw new RuntimeException(e.getMessage());
		}
	}
