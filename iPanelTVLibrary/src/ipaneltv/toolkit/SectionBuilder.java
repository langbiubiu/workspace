...
public class SectionBuilder {
	...
	public static int buildPATSection(int stream_id, int version, int[] pns, int[] pids, byte[] b) {
		int n = pns.length;
		int off = 8;
		if (pids.length != n || n <= 0)
			throw new IllegalArgumentException();
		/*
		 * section_syntax_indicator(1)+private_indicator(1)+reserved(2)+
		 * section_length(12)
		 */
		int seclen = 12 + n * 4;// header(8B) + pn * 4B + CRC(4B)
		int slen = seclen - 3;// 3B(begin from third byte)
		b[0] = 0x00;// table id
		b[1] = (byte) (0xb0 | (slen >> 8));// 0xb0=10110000
		b[2] = (byte) slen;

		/* table_id_externtion = transport_stream_id */
		b[3] = (byte) (stream_id >> 8);
		b[4] = (byte) stream_id;

		/* reserved(2)+version_number(5)+current_next_indicator(1) */
		b[5] = (byte) (0xc1 | ((version & 0x1f) << 1));
		...
		for (int i = 0; i < n; i++) {
			int boff = off + i * 4;
			b[boff + 0] = (byte) (pns[i] >> 8);
			b[boff + 1] = (byte) pns[i];
			b[boff + 2] = (byte) ((byte) (pids[i] >> 8) | 0xE0);
			b[boff + 3] = (byte) pids[i];
		}

		int crc = calculateCRC32(b, seclen - 4);
		b[seclen - 4] = (byte) (crc >> 24);
		b[seclen - 3] = (byte) (crc >> 16);
		b[seclen - 2] = (byte) (crc >> 8);
		b[seclen - 1] = (byte) (crc);
		return seclen;
	}

	public static int buildPMTSection(int program_number, int version, int pcr_pid, 
			int[] csids, int[] scpids,int[] stypes,int spids[], byte[] b) {
		int n = stypes.length, off = 12 ,m = 0;
		if(csids != null){
			m = csids.length;
			if(scpids.length != m || m <= 0)
				throw new IllegalArgumentException();
		}
		if (spids.length != n || n <= 0)
			throw new IllegalArgumentException();
		 ...
		 * section_length(12)
		 */
		int seclen = 16 + m * 6 + n * 5; // header(12B) + caid * 6B +pn * 5B + CRC(4B)
		int slen = seclen - 3; // 3B(begin from third byte)
		b[0] = 0x02; /* table_id */
		b[1] = (byte) (0xb0 | (slen >> 8));
		b[2] = (byte) slen;
		b[3] = (byte) (program_number >> 8);
		b[4] = (byte) program_number;

		/* reserved(2)+version_number(5)+current_next_indicator(1) */
		b[5] = (byte) (0xc1 | ((version & 0x1f) << 1));
		b[6] = 0;/* section_number(8) */
		b[7] = 0;/* last_section_number(8) */

		/* reserved(3)+pcr_pid(13) */
		b[8] = (byte) (0xe0 | ((pcr_pid >> 8) & 0x1f));
		b[9] = (byte) (pcr_pid);
		...
		if(m != 0){
			for(int j = 0; j < m; j++){
				...
				b[coff + 0] = (byte)0x09;
				b[coff + 1] = (byte)0x04;
				b[coff + 2] = (byte)(csids[j] >> 8);
				b[coff + 3] = (byte)csids[j];
				b[coff + 4] = (byte)(0xe0 | (scpids[j] >> 8));
				b[coff + 5] = (byte)(scpids[j]);
			}
		}
		
		for (int i = 0; i < n; i++) {
			int boff = off + m * 6 + i * 5;
			b[boff + 0] = (byte) stypes[i];/* stream_type(8) */
			b[boff + 1] = (byte) (0xe0 | (spids[i] >> 8));/*
														 * reserved(3)+
														 * elementary_PID(13)
														 */
			b[boff + 2] = (byte) spids[i];
			b[boff + 3] = (byte) 0xf0;/* reserved(4)+ES_info_length(12) */
			b[boff + 4] = 0;
		}
	...
	public static int buildTSSection(int pid, byte[] sbuf, int slen,
			int ccounter, byte[] outbuf) {
		int paysize = 0;
		int num = 0;
		int off = 0, soff = 0;
		int outlen = outbuf.length;

		if (outlen < ((slen + 181) / 182) * 188)
			return -1;
		...
		for (int i = 0; i < num; i++) {
			paysize = slen - soff;
			if (i == 0) {
				paysize = (183 < paysize) ? 183 : paysize;
			} else {
				paysize = (184 < paysize) ? 184 : paysize;
			}

			outbuf[soff + 0] = 0x47;/* sync_byte */
			/*
			 * transport_error_indicator(1)+payload_unit_start_indictor(1)+
			 * transport_priority(1)+PID(13)
			 */
			if (i == 0)
				outbuf[soff + 1] = (byte) (0x40 | (byte) (pid >> 8));
			else
				outbuf[soff + 1] = (byte) (0x0 | (byte) (pid >> 8));
			outbuf[soff + 2] = (byte) pid;

			/*
			 * transport_scrambling_control(2)+adaptation_field_control(2)+
			 * continuity_counter(4)
			 */
			outbuf[soff + 3] = (byte) (0x10 | ((ccounter) & 0x0f));
			off = 4;
			if (i == 0) {
				outbuf[soff + 4] = 0;/* pointer_field */
				off = 5;
			}

			for (int j = 0; j < paysize; j++) {
				outbuf[soff + off + j] = sbuf[j];
			}
			...
			if (paysize + off < 188) {
				for (int m = 0; m < (188 - (paysize + off)); m++) {
					outbuf[soff + m] = (byte) 0xff;
				}
			}
		}
		return 0;
	...
	public static int calculateCRC32(byte[] b, int len) {
		int i;
		int crc = 0xffffffff;
		for (i = 0; i < len; i++)
			crc = (crc << 8) ^ crc_table[((crc >> 24) ^ b[i]) & 0xff];
		return crc;
	}

	final static int crc_table[] = new int[] {//
			//
