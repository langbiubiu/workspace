package com.ipanel.join.cq.user.chasedrama;
...
public class ChaseDramaFragment extends BaseFragment {
	...
	TextView user_chase_curpage,user_chase_totalpage,chase;
	int count = 0;
	int num = 0;
	LinearLayout chase_drama_linear;
	View title;
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_COLUME_DATA_LOADED:
				...
				viewPager.setAdapter(new VODPagerAdapter());
				if(totalMovieNumber > 0 ){
					chase.setVisibility(View.VISIBLE);
				}
				user_chase_totalpage.setText(getTotalPageSize()+"");
				break;
			//一页影片数据加载完成	
			...
			case MSG_REQUEST_TIME_UP:
				...
				break;
			case MSG_REFOCUS:
				chase_drama_linear.requestFocus();
				break;
			}
		}
	};
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		return inflater.inflate(R.layout.user_chase_fragment, container, false);		
	}
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		chase_drama_linear = myActivity.chase_drama_linear;
		mImageFetcher = SharedImageFetcher.getNewFetcher(getActivity(), 3);
		view = getView();
		serviceHelper = ServiceHelper.getHelper();
		viewPager = (VerticalViewPager2)view.findViewById(R.id.chase_gd);
		title = view.findViewById(R.id.user_chase_title);
		chase = (TextView) view.findViewById(R.id.chase);
		user_chase_curpage = (TextView) view.findViewById(R.id.user_chase_curpage);
		user_chase_curpage.setText("0/");
		user_chase_totalpage = (TextView) view.findViewById(R.id.user_chase_totalpage);
		initControl();
	...
	class VODPagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, final int position) {
			...
			if (itemView == null) {
				itemView = View.inflate(getActivity().getBaseContext() , R.layout.user_chase_griditem, null);
			}
			WeightGridLayout mWeightLayout = (WeightGridLayout) itemView.findViewById(R.id.chasegrid);
	...
	private void initData(){
		...
		request.getParam().setPagesize(20);
			...
			public void onResponse(boolean success, GetHwResponse result) {
				if(success && result.getError().getCode() == 0){
					...
					for(int i=0;i<total;i++){
						if(list.get(i).getModel().equals("teleplay"))
	...
	private void refeshWeightLayoutChild(RowRequestParam param, WeightGridLayout layout){
		...
		for (int i = param.view_start; i < end; i++){
			...
			final ImageView image = (ImageView) convertView.findViewById(R.id.series_pic);
			final TextView name = (TextView) convertView.findViewById(R.id.series_name);
			final TextView num = (TextView) convertView.findViewById(R.id.series_num);
...
//			int idx =  Integer.parseInt(item.series_idx);
//			int curNum = item.get - idx;
//			num.setText(curNum+"");
		}
	}
	...
	class VODWeightGridAdapter extends WeightGridAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			if (convertView == null) {
				convertView = LayoutInflater.from(getActivity().getApplication()).inflate(R.layout.user_chase_pageritem, parent, false);
			}
			final ImageView image = (ImageView) convertView.findViewById(R.id.series_pic);
			final TextView name = (TextView) convertView.findViewById(R.id.series_name);
			final FrameLayout user_uninstall_fl = (FrameLayout) convertView.findViewById(R.id.user_uninstall_fl3);
			final ImageView del = (ImageView) convertView.findViewById(R.id.user_delete3);
			mImageFetcher.setImageSize(241, 300);
...
//						HomedDataManager.openTVDetail(ctx,item.id+"",item.name,item.series_idx,null);					
					}
				}
			});
			convertView.setOnFocusChangeListener(new OnFocusChangeListener() {
				...
				public void onFocusChange(View v, boolean hasFocus) {
					if (hasFocus) {
						user_chase_curpage.setText(row+1+"/");
						if(isUninstallState){
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		case KeyEvent.KEYCODE_BACK:
			if(isUninstallState){				
				FrameLayout user_uninstall_fl = (FrameLayout) curView.findViewById(R.id.user_uninstall_fl3); 
				user_uninstall_fl.setVisibility(View.GONE);
				isUninstallState = false;
				return true;
			}
      ...
      // 设置好参数之后再show
		...
		delOne.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View mov) {												
				...
				//TODO 
				FrameLayout user_uninstall_fl = (FrameLayout) v.findViewById(R.id.user_uninstall_fl3); 
				if(user_uninstall_fl==null){
					return;
				}else{
					user_uninstall_fl.setVisibility(View.VISIBLE);
					ImageView user_delete2 = (ImageView) v.findViewById(R.id.user_delete3);
	...
	protected  void deleteAll(final View v) {
		...
		user_del_tips.setText(R.string.del_tips_chase);
		final View title = view.findViewById(R.id.user_chase_title);
		title.setFocusable(true);
		title.requestFocus();
