...
public abstract class JHListView extends RelativeLayout {
	...
	public int selector_res = android.R.color.transparent;
	...
	public JHListView(Context context) {
		super(context);
	...
	public JHListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.context = context;
		current_state = STATE_OF_UN_READY;
		this.setOnKeyListener(new OnKeyListener() {
	...
	public void init() {
		...
		if(show_count!=child_count-2){
			...
			for (int i = 0; i < show_count; i++) {
				...
				views.add(view);
//				this.addView(view);
				this.addViewInLayout(view, this.getChildCount()-1, view.getLayoutParams());
			}
			...
			this.addViewInLayout(tmpView, this.getChildCount()-1, tmpView.getLayoutParams());
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		int height = list_item_height;
		int width = list_item_width * show_count + (show_count - 1) * spacing;
	...
	public void doSpecialAnim(boolean isFirst) {
		...
		animation = new TranslateAnimation(Animation.RELATIVE_TO_SELF, isFirst ? -0.05f : 0.05f,
				Animation.RELATIVE_TO_SELF, isFirst ? -1.0f : 1.0f, Animation.RELATIVE_TO_SELF,
	...
	private int getLeftMargin(int index) {
		int result=index * list_item_width + index * spacing;
		if(this.getCurrentIndex()-selector_index==0){
			if(index>2){
				result-=45*(index-2);
			}
		}else if(this.getCurrentIndex()-selector_index==1){
			if(index>1){
				result-=45*(index-1);
			}
		}else{
