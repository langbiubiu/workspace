...
public class JEventListView extends JListView {
	
	public List<LiveProgramEvent> shows = new ArrayList<LiveProgramEvent>();
	int mFocusColor = Color.parseColor("#000000");
	int mNormalColor = Color.parseColor("#ffffff");
	int mSelectColor = Color.parseColor("#ffba00");
	...
	public JEventListView(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
		mActivity = (LiveActivity)context;
	}

	public JEventListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		// TODO Auto-generated constructor stub
		mActivity = (LiveActivity)context;
	}
	...
	public void init() {
		...
		item_resourceId = R.layout.live_item_program_01;
		list_item_width = 497;
	...
	public View getListView(int position, View convertView) {
		...
		if (isbooked) {
			book_mark.setImageResource(R.drawable.live_icon000);
		...
		if(position==this.getCurrentIndex() && hasFocus){
			...
			if (isbooked)
				book_mark.setImageResource(R.drawable.live_icon_book);
		}
		return convertView;
	}
	...
	public void setBookState(BookData book,boolean flag){
		for(int i=0;i<shows.size();i++){
			if(shows.get(i).getChannelKey().getProgram()==book.getProgram_number()&&(shows.get(i).getStart()+"").equals(book.getStart_time())){
				shows.get(i).status = (flag?1:0);
			}
		}
	}
