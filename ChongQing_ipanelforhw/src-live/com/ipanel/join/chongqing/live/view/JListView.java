...
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.AnimInEndListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.AnimInStartListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.AnimOutEndListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.AnimOutStartListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.ListDataEmptyListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.ListDataLoadTimeUp;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.ListDataLoadingCallBack;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.ListDataLoadingListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.ListFocusChangeListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.ListKeyPressgListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.ListKeyUpListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.ListOnEnterListener;
import com.ipanel.join.chongqing.live.view.ListViewListenerSet.ListPreFocusChangeListener;


public abstract class JListView extends RelativeLayout {
	...
	public int list_item_height = 70;
	public int list_item_width = 175;
	public int spacing = 3;

	public int show_count = 5;
	public int data_count;

	public int center_index;
	public int current_index = 0;
	public int selector_index;
//	public int selector_res = R.drawable.live_selector;
	...
	public int defaultAnimDelay = 200;
	public int defaultAnimTime = 60;// 上下动画时间
	public int defaultCenter = -1;
	public float rate = 1;
	...
	public ListKeyUpListener key_up_listener;
		...
		public void handleMessage(android.os.Message msg) {
			...
			case 2:
				...
				if (anim_type == 0) {
					...
					// animation.reset();
					// views.get(in_anim).startAnimation(animation);
			...
			case 3:
				...
				if (anim_type == 0) {
					...
					// animation.setFillAfter(true);
					// animation.reset();
					// views.get(show_count - out_anim - 1).startAnimation(
					// animation);
	...
	public void init() {
		...
		rate = 1.0f + (spacing + 0.0f) / (list_item_height + 0.0f);
		Log.e(TAG, rate + "");
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		int width = list_item_width;
		int height = list_item_height * show_count + (show_count - 1) * spacing;
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		if(key_up_listener!=null){
			return key_up_listener.onKeyUp(keyCode);
		}
		return false;
	}
...
//					animUp(-1);
				...
				if (focus_change_listener != null)
					doChangeOperation();
				if (pre_focus_change_listener != null){
					pre_focus_change_listener.onPreFocusChange(getCurrentIndex());
				}
			}
			return true;
...
//					animDown(-1);
		...
		case KeyEvent.KEYCODE_DPAD_CENTER:
			...
			break;
		case RcKeyEvent.KEYCODE_PAGE_UP:
		case RcKeyEvent.KEYCODE_CH_UP:
			...
			return true;
		case RcKeyEvent.KEYCODE_PAGE_DOWN:
...
//			}
	...
	public void listLayout() {
		for (int i = 0; i < show_count; i++) {
			...
			tmp.setLayoutParams(getLayoutParams(tmp, i));
		}
	}
...
//			tmp.setLayoutParams(getLayoutParams(tmp, i));
	...
	public RelativeLayout.LayoutParams getLayoutParams(View view, int index) {
		...
		p.topMargin = getTopMargin(index);
		p.leftMargin = 0;
		return p;
	}

	private int getTopMargin(int index) {
		return index * list_item_height + index * spacing;
	}
	...
	public void setCurrentIndexNCircle(int index){
		...
		if(data_count > show_count){
			if((data_count - 1 - index ) >= (show_count -1)){
				setCurrentIndex(index);
				selector_index = 0;
			}else{
				first_show = data_count - show_count;
				setCurrentIndex(first_show);
				current_index = first_show;
				selector_index = index - first_show;
			}
		}else{
			setCurrentIndex(0);
	...
	public void setListKeyUpListener(ListKeyUpListener listener){
		this.key_up_listener=listener;
	}
	...
	public void setListKeyPressgListener(ListKeyPressgListener listener) {
		this.press_listener = listener;
	}
