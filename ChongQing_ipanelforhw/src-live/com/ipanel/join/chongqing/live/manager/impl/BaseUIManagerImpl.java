 ...
 * 界面部件的管理类
 * */
public abstract class BaseUIManagerImpl extends UIManager {
	protected final static String TAG = BaseUIManagerImpl.class.getSimpleName();

	/** 配置列表 */
	protected final HashMap<Integer, UIConfig> config = new HashMap<Integer, UIConfig>();
	...
	public BaseUIManagerImpl(IManager cxt, int root, int mask) {
		this.root = root;
		this.mask = mask;
	...
	/** 根据id显示界面 */
	public void showUI(final int id, final Object o) {
		Log.d("lixby", "os will show Ui  id  :"+id);
		final UIConfig ui = config.get(id);
		if (ui == null) {
			LogHelper.e(TAG,
	...
	public boolean detatchExclusiveFagment() {
		...
		if (rf != null && rf.isAdded()) {
			...
			LogHelper.i(TAG, String.format("detach fragment : %s", rf
	...
	public boolean handleKeyEvent(int keyCode, KeyEvent event) {
		LogHelper.i("UIFragment handle key event, keycode = " + keyCode);
	 ...
	 * 判断某个界面是否正在显示
	 * */
	public boolean isFragmentAdded(int UID) {
		UIConfig ui = getConfigUI(UID);
		if (ui != null) {
			UIFragment f = getFragmentByUIID(ui.getUID());
