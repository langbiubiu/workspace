...
public class AllPFDataGetter {
	...
	HandlerThread thread = new HandlerThread("threan");
	...
	OtherPFDataGetter otherPFDataGetter;
	...
	public AllPFDataGetter(Context context, String uuid, int flags) {
		...
		otherPFDataGetter = new OtherPFDataGetter(context, uuid, flags) {
			@Override
			protected void onProgramsFound(final PresentAndFollow pf, int flags) {
				postProc(new Runnable() {

					@Override
					public void run() {
						IPanelLog.d(TAG, "otherPFDataGetter onProgramsFound pf = " + pf
								+ ";focusChannel = " + focusChannel);
						if (focusChannel != null) {
							try {
								if (pf != null && pf.getChannelKey() != null
										&& pf.getChannelKey().equals(focusChannel)) {
									IPanelLog.d(TAG, "otherPFDataGetter onProgramsFound focusChannel = "+ focusChannel);
	...
	public void getPresentAndFollow(final long freq, final int pro, final int flags) {
		postProc(new Runnable() {
			@Override
			public void run() {
				Log.d(TAG, "getPresentAndFollow pro = " + pro);
				...
				if (pf == null) {
					...
					map.put(key, pf);
					otherPFDataGetter.getPresentAndFollow(freq, pro, flags);
				} else {
					...
					if (program != null) {
						...
						if (!(startTime < currentTime && endTime > currentTime)) {
							Log.d(TAG, "getPresentAndFollow 11 pro = " + pro + ";try to search");
							otherPFDataGetter.getPresentAndFollow(freq, pro, flags);
						}
					}else{
						otherPFDataGetter.getPresentAndFollow(freq, pro, flags);
						Log.d(TAG, "getPresentAndFollow 22 pro = " + pro + ";try to search");
					}
				}
				focusChannel = ChannelKey.obten(freq, pro);
				postProgramsFound(pf, 0);
			}
		});
	}
	...
	public static class PresentAndFollow {
		ChannelKey key;
		Program present;
		Program follow;
