...
public class NgodPlayManager implements PlayInterface {
	...
	private LivePFDatagetter livePFDatagetter = null;
		...
		postProc(new Runnable() {
			...
			public void run() {
				...
				try {
				    ...
				    //Log.d(TAG, "duration=" + shiftration.duration + ",t=" + millis + ",start="+ shiftration.start);
	...
	void procUpdateSelectResult() {
		Log.d(TAG, "procUpdateSelectResult 2 isLiveMode = "+ isLiveMode()+";isSelecting = "+ isSelecting+";selection = "+ selection);
		if (isLiveMode()) {
			if (isSelecting) {
				isSelecting = false;
				if (selection != null) {
					DvbLivePlayer p = host.tryGetLivePlayer();
					Log.d(TAG, "procUpdateSelectResult 3 p = " + p);
					if (p != null) {
	...
	public synchronized void getPresentAndFollow(final ChannelKey ch) {
		Log.d(TAG, "getPresentAndFollow freq=" + ch.getFrequency() + ",pn=" + ch.getProgram() +";livePFDatagetter = "+ livePFDatagetter);
	...
	private void init(){
		if (livePFDatagetter == null) {
			Log.d(TAG, "getPresentAndFollow in 111");
			livePFDatagetter = new LivePFDatagetter(host.getActivity().getApplicationContext(),
					host.getNetworkUUID(), 0);
					Log.d(TAG, "getPresentAndFollow 111");
//			 livePFDatagetter.setEncoding("gbk");
			 Log.d(TAG, "getPresentAndFollow 222");
			livePFDatagetter.setAttention(true, 0);
			Log.d(TAG, "getPresentAndFollow");
			livePFDatagetter.setListener(new LivePFDatagetter.PFListener() {
				@Override
				public void onPfInfoUpdated(final PresentAndFollow pf) {
					// TODO Auto-generated method stub
					Log.d(TAG, "onPfInfoUpdated pf = " + pf );
//					Log.d(TAG,
//							present == null ? "present is null" : "present.getName()"
//									+ present.getName());
//					Log.d(TAG,
//							follow == null ? "follow is null" : "follow.getName()"
//									+ follow.getName());
					postUI(new Runnable() {
						@Override
						public void run() {
...
class NgodShiftSource extends TsPlayerInetSource {
	...
	public NgodShiftSource(NgodPlayManager manager) {
