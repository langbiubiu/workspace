...
public abstract class UIFragment extends Fragment implements IUIControl {
	/** 隐藏界面的消息 */
	public static final int MSG_HIDE_UI = 999;
	public String TAG = "";
	...
	private View findNextFocus(int direction) {
		...
		try{
			View v= FocusFinder.getInstance().findNextFocus((ViewGroup) getView(),
					getActivity().getCurrentFocus(), direction);
			if(caculateFocusable(v)){
				return v;
			}
		}catch(Exception e){
			e.printStackTrace();
	...
	protected boolean caculateFocusable(View v){
		return true;
	}
