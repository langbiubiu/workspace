...
public class HTTPRequest extends HTTPPacket
{
	...
	private MessageEvent nettyEvent;
	public HTTPRequest(ChannelHandlerContext ctx, MessageEvent e){
		initFromNetty(ctx, e);
	}

	protected void initFromNetty(ChannelHandlerContext ctx, MessageEvent e) {
		this.nettyCtx = ctx;
		this.nettyEvent = e;
        ...
        setContent(content.toString(CharsetUtil.UTF_8));
	}
	...
	{
		if(nettyEvent != null){
			InetSocketAddress so = (InetSocketAddress) nettyEvent.getChannel().getLocalAddress();
			return so.getAddress().getHostAddress();
		}
	...
	public boolean post(File file, String contentType){
		if(nettyCtx != null){
			try {
				NettyHTTPServer.sendFile(nettyCtx, nettyEvent, file, contentType);
				return true;
			} catch (Exception e) {
	...
	{
		if(nettyCtx != null){
			NettyHTTPServer.sendResponse(nettyCtx, nettyEvent, httpRes);
			return true;
		}
	...
	{
		if(httpReq.nettyCtx != null){
			initFromNetty(httpReq.nettyCtx, httpReq.nettyEvent);
		} else {
	...
	public String toString()
	{
		StringBuffer str = new StringBuffer();
		if(nettyCtx != null){
			str.append(nettyCtx.getChannel().getRemoteAddress().toString());
