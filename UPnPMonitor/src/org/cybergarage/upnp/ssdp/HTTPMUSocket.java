...
//		}
	...
	public boolean open(String addr,int port, InetAddress bindAddr){
		try {
			...
			Logger.d("create HTTPMU socket on: "+ssdpMultiGroup.getAddress().getHostAddress()+":"+port);

			ssdpMultiIf = NetworkInterface.getByInetAddress(bindAddr);
			//ssdpMultiSock.joinGroup(ssdpMultiGroup.getAddress());
	...
	public boolean close()
	{
		...
		try {
			ssdpMultiSock.leaveGroup(ssdpMultiGroup.getAddress());
            ssdpMultiSock.close();
			ssdpMultiSock = null;
		}
		catch (Exception e) {
