...
public class ArgumentList extends Vector 
{
	...
	public void setResArgs(ArgumentList outArgList)
	{
        ...
        for (int n=0; n<nArgs; n++) {
            Argument arg = getArgument(n);
            System.out.println("setResArgs arg direction = " + arg.getDirection());
            if (arg.isOutDirection()){
	            String argName = arg.getName();
	            System.out.println("setResArgs arg name = " + arg.getName());
	            Argument outArg = outArgList.getArgument(argName);
	            if (outArg == null) System.out.println("setResArgs outArg is null");
	            if (outArg != null ) {
	            	System.out.println("setResArgs outArg value " + outArg.getValue());
	            	arg.setValue(outArg.getValue());
	            }
	            /*
	             * service中的action中与请求到的ArgumentResponse列表中，存在Argument的name不一致，抛出异常后会导致循环中断，导致后续的Argument无法继续匹配，故注释掉
	             */
//	            if (outArg == null)
//	                throw new IllegalArgumentException("Argument \"" + argName + "\" missing.");
            }
