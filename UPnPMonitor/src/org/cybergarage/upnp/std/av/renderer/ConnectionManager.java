...
public class ConnectionManager implements ActionListener, QueryListener
{
	...
	public ConnectionManager(MediaRenderer render)
	{
		...
		addConnectionInfo(new ConnectionInfo(maxConnectionID));
		updateStateVariable(CURRENTCONNECTIONIDS, maxConnectionID+"");
	}
	...
	public boolean actionControlReceived(Action action)
	{
		...
		if (actionName.equals(GETPROTOCOLINFO) == true) {
			...
			// Sink
			String[] formats = new String[]{"audio/mp3","audio/mpeg","video/mpeg","video/mp4","video/m4v","image/jpeg","image/png","image/gif"};
			StringBuilder sb = new StringBuilder();
			for(String f: formats){
				if(sb.length()>0)
					sb.append(',');
				sb.append(String.format(HTTP_GET + ":*:%s:*", f));
			}
			action.getArgument(SINK).setValue(sb.toString());
			return true;
		}
	...
	public StateVariable getStateVariable(String name){
		return mediaRenderer.getStateVariable(SERVICE_TYPE,
				name);
	}
