...
public class AVTransport implements ActionListener, QueryListener
{
	...
	public final static String ABS_TIME = "ABS_TIME";
	...
	public AVTransport(MediaRenderer render)
	{
		...
		lastChange = new Event();
		updateStateVariable(getStateVariable(AVTransport.TRANSPORTSTATE),
				AVTransport.STOPPED);
		updateStateVariable(getStateVariable(AVTransport.TRANSPORTSTATUS),
				AVTransport.OK);
		updateStateVariable(getStateVariable(AVTransport.CURRENTTRANSPORTACTIONS), PLAY + ","
				+ STOP + "," + SEEK + "," + PAUSE+", X_DLNA_SeekTime");
	}
	
	public StateVariable getStateVariable(String name){
		return mediaRenderer.getStateVariable(AVTransport.SERVICE_TYPE,
				name);
	}
	...
	public boolean actionControlReceived(Action action)
	{
		try {
			...
			if (actionName.equals(SETAVTRANSPORTURI) == true) {
				...
				Argument arg = action.getArgument(CURRENTURI);
				arg.getRelatedStateVariable().setValue(arg.getValue());

				arg = action.getArgument(CURRENTURIMETADATA);
				arg.getRelatedStateVariable().setValue(arg.getValue());
			...
			if (actionName.equals(GETCURRENTTRANSPORTACTIONS)) {
				action.getArgument(ACTIONS).setValue(
						getStateVariable(CURRENTTRANSPORTACTIONS).getValue());
				isActionSuccess = true;
			}
			...
			if (actionName.equals(GETMEDIAINFO) == true) {
				action.getArgument(NRTRACKS).setValue(avTransInfoList.size());
				action.getArgument(PLAYMEDIUM).setValue("NONE");
				synchronized (avTransInfoList) {
					int avTransInfoCnt = avTransInfoList.size();
					if(avTransInfoCnt > 0){
						AVTransportInfo avTransInfo = avTransInfoList.getAVTransportInfo(0);
						if(avTransInfo != null){
						...
						if(avTransInfoCnt > 1){
							avTransInfo = avTransInfoList.getAVTransportInfo(1);
							if(avTransInfo != null){
								action.getArgument(NEXTURI).setValue(avTransInfo.getURI());
			 ...
			 * 
			 * if (actionName.equals(CONNECTION_COMPLETE) == true) { return
			 * true; }
			 * 
			...
			if (dmr != null && !isActionSuccess) {
				ActionListener listener = dmr.getActionListener();
				if (listener != null)
					return listener.actionControlReceived(action);
			}
	...
	{
		Logger.d(AVTransport.class.getSimpleName()+": "+stateVar);
		return false;
	}
	
	public void updateStateVariable(String name, String val){
		updateStateVariable(getStateVariable(name), val);
	}
	
	public void updateStateVariable(StateVariable sv, String val){
		if(sv == null)
			return;
		sv.setValue(val);
		lastChange.addProperty(sv.getName(), sv.getValue());
		getStateVariable(LASTCHANGE).setValue(lastChange.toNode().toString());
	}
}
