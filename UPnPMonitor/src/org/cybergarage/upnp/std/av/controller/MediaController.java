...
public class MediaController extends ControlPoint
{
	...
	public Action getTransportInfo(Device dev){
		Action action = getAction(dev, AVTransport.SERVICE_TYPE, AVTransport.GETTRANSPORTINFO);
		if(action != null){
			action.setArgumentValue(AVTransport.INSTANCEID, "0");
	...
	public Action getMediaInfo(Device dev){
		Action action = getAction(dev, AVTransport.SERVICE_TYPE, AVTransport.GETMEDIAINFO);
		if(action != null){
			action.setArgumentValue(AVTransport.INSTANCEID, "0");
			action.postControlAction();
		}
	...
	public Action getPositionInfo(Device dev){
		Action action = getAction(dev, AVTransport.SERVICE_TYPE, AVTransport.GETPOSITIONINFO);
		if(action != null){
			action.setArgumentValue(AVTransport.INSTANCEID, "0");
			action.postControlAction();
		}
	...
	public Action Seek(Device dev, String target){
		Action action = getAction(dev, AVTransport.SERVICE_TYPE, AVTransport.SEEK);
		if(action != null){
			action.setArgumentValue(AVTransport.INSTANCEID, "0");
			action.setArgumentValue(AVTransport.UNIT, AVTransport.REL_TIME);
	...
	public Action getAction(Device dev, String service, String action){
		if (dev == null)
			return null;
		
		Service avTransService = dev.getService(service);
		if (avTransService == null)
			return null;
		
		return avTransService.getAction(action);
	}
