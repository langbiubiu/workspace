...
public abstract class ImageWorker {
	...
	public void loadImage(Executor executor, ImageFetchTask data,
		...
		if (mImageCache != null) {
			bitmap = mImageCache.getBitmapFromMemCache(data.getStoreKey());
		}
	 ...
	 */
	protected abstract Bitmap processBitmap(ImageFetchTask data);
	...
	private class BitmapWorkerTask extends
			AsyncTask<ImageFetchTask, Void, Bitmap> {
		 ...
		 */
		@Override
		protected Bitmap doInBackground(ImageFetchTask... params) {
			...
			if (mImageCache != null && !isCancelled() && !mExitTasksEarly) {
				bitmap = mImageCache.getBitmapFromDiskCache(data.getStoreKey());
			}
			...
			if (bitmap != null && mImageCache != null) {
				mImageCache.addBitmapToCache(data.getStoreKey(), bitmap);
			}

			if (Utils.DEBUG) {
				Log.d(TAG,
						"doInBackground - finished work " + data.getImageUrl());
			}
